...
# 通过主函数中的get_performance调用，计算感兴趣的实体预测值的RMSE和MAE
...
def estimate_params(edge_list, x, rel_to_scale = None, u_0 = None):
  ...
  for p in range(P):
    edges = edge_list[p]
    # 为每个pair生成一个T/F，当两个条目都已知时为T
    known_pairs = u_0[edges[:,1]] & u_0[edges[:,0]]
    #print(known_pairs)
    if sum(known_pairs) > 1:
      x_pairs = x[edges[known_pairs]] # 提取为true的那些属性值
      if rel_to_scale is not None:
...
# 从edge_list里提取了pair是同一属性的条目，并记录了其edge_list的item、编号和对应属性名
...
def extract_edges_YAGO(triples, entities, inner_edge = True):
  ...
  # 构造每个属性的dic，key为attr，v为每个属性的三元组table
  ...
  if inner_edge:
    for comb in att_combs: # 不同的两种属性
      ...
      if any(overlap):
        ...
        ent_1 = att_ent_dict[comb[0]]
        ent_2 = att_ent_dict[comb[1]]
        # overlap[overlap]是提取了其中为true的条目
  ...
  # edge_list存的就是[n1,n2]的numpy数组，其中n1、n2都是index而不是node名，是对应属性的索引id
...
def find_prim_sec_atts(entities):
  ...
  att_coexist = find_coexist_atts(entities) # 基于最开始的属性group，得到被共有的属性组
  #print(att_coexist) # [{'people.person.date_of_birth', 'people.deceased_person.date_of_death'}, {'film.film.initial_release_date'}, {'organization.organization.date_founded'}, {'location.dated_location.date_founded'}]
...
def extract_edges_FB(triples, entities, inner_edge = True):
  ...
  def triples_entities_to_edgelist(triples, entities, edge_list, relations):
    ...
    """
    # 这两行就是把triples里的node_1和node_2从名字改成了id，pandas的操作很慢
